...
class TargetManagementApiTest(TestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('manage_target')
    ...
    def test_update_existing_target(self):
        ...
        self.assertEqual(float(target.target), 1500.0)
...
class GetTargetApiTest(TestCase):
    ...
    def test_get_target_success(self):
        ...
        self.assertEqual(float(response.data['target']), 1000.0)
...
class GetScoreHistoryApiTest(TestCase):
    ...
    def test_get_history_with_targets(self):
        ...
        Target.objects.create(user=self.user, target=1000, month=1, year=2024, score=80)
        Target.objects.create(user=self.user, target=1200, month=2, year=2024, score=90)
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_get_history_pagination(self):
        ...
        for i in range(25):
            Target.objects.create(user=self.user, target=1000 + i, month=i % 12 + 1, year=2024, score=0)
