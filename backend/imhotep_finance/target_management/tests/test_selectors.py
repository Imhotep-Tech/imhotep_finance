...
from target_management.selectors import get_target_for_user, get_all_targets_for_user, get_latest_target_for_user
...
class GetTargetForUserTest(TestCase):
    ...
    def test_get_target_specific_month(self):
        ...
        today = date.today()
        target = get_target_for_user(user=self.user, month=today.month, year=today.year)
        self.assertIsNotNone(target)
...
class GetAllTargetsForUserTest(TestCase):
    ...
    def test_get_all_targets_ordered(self):
        ...
        Target.objects.create(user=self.user, target=1000, month=1, year=2024, score=0)
        Target.objects.create(user=self.user, target=1200, month=3, year=2024, score=0)
        Target.objects.create(user=self.user, target=1100, month=2, year=2024, score=0)
        targets = get_all_targets_for_user(user=self.user)
...
class GetLatestTargetForUserTest(TestCase):
    ...
    def test_get_latest_target(self):
        ...
        from target_management.models import Target
        Target.objects.create(user=self.user, target=1000, month=1, year=2024, score=0)
        latest = Target.objects.create(user=self.user, target=1500, month=3, year=2024, score=0)
