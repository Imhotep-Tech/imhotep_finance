...
class GetScheduledTransactionsForUserTest(TestCase):
    ...
    def setUp(self):
        ...
        self.active_st = create_scheduled_transaction(user=self.user, day_of_month=15, amount=500, currency='USD', scheduled_trans_status='deposit', category='Salary', scheduled_trans_details='Active')
        self.inactive_st = create_scheduled_transaction(user=self.user, day_of_month=20, amount=300, currency='USD', scheduled_trans_status='withdraw', category='Bills', scheduled_trans_details='Inactive')
