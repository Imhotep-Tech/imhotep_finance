...
class ScheduledTransactionCreateApiTest(TestCase):
    ...
    def test_create_scheduled_transaction_success(self):
        ...
        data = {'day_of_month': 15, 'amount': '500.00', 'currency': 'USD', 'scheduled_trans_status': 'deposit', 'category': 'Salary', 'scheduled_trans_details': 'Monthly salary'}
    ...
    def test_create_scheduled_transaction_invalid_data(self):
        ...
        data = {'day_of_month': 32, 'amount': '500.00', 'currency': 'USD', 'scheduled_trans_status': 'deposit'}
        response = self.client.post(self.url, data, format='json')
...
class ScheduledTransactionListApiTest(TestCase):
    ...
    def setUp(self):
        ...
        for i in range(3):
            create_scheduled_transaction(user=self.user, day_of_month=i + 1, amount=100 * (i + 1), currency='USD', scheduled_trans_status='deposit', category=f'Category{i}', scheduled_trans_details='')
...
class ApplyScheduledTransactionsApiTest(TestCase):
    ...
    def test_apply_scheduled_transactions(self):
        ...
        create_scheduled_transaction(user=self.user, day_of_month=1, amount=100, currency='USD', scheduled_trans_status='deposit', category='Test', scheduled_trans_details='')
        response = self.client.post(self.url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
