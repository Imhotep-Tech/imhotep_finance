...
class WishCreateApiTest(TestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('create_wish')

    def test_create_wish_success(self):
        """Test creating wish via API"""
        data = {'price': '500.00', 'currency': 'USD', 'year': 2024, 'wish_details': 'New laptop', 'link': 'https://example.com'}
    ...
    def test_create_wish_unauthenticated(self):
        ...
        data = {'price': '500.00', 'currency': 'USD', 'year': 2024, 'wish_details': 'Test', 'link': ''}
        response = self.client.post(self.url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_create_wish_invalid_data(self):
        """Test creating wish with invalid data"""
        data = {'price': '-100', 'currency': 'USD', 'year': 2024}
        response = self.client.post(self.url, data, format='json')
...
class WishDeleteApiTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(username='testuser', password='testpass123')
        self.client.force_authenticate(user=self.user)
        self.wish = create_wish(user=self.user, price=500, currency='USD', year=2024, wish_details='Test', link='', place='General')
...
class WishUpdateApiTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(username='testuser', password='testpass123')
        self.client.force_authenticate(user=self.user)
        self.wish = create_wish(user=self.user, price=500, currency='USD', year=2024, wish_details='Original', link='', place='General')
        self.url = reverse('update_wish', kwargs={'wish_id': self.wish.id})

    def test_update_wish_success(self):
        """Test updating wish via API"""
        data = {'price': '600.00', 'currency': 'EUR', 'year': 2025, 'wish_details': 'Updated', 'link': 'https://updated.com'}
        response = self.client.post(self.url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.wish.refresh_from_db()
        self.assertEqual(float(self.wish.price), 600.0)
...
class GetWishlistApiTest(TestCase):
    ...
    def setUp(self):
        ...
        for i in range(5):
            create_wish(user=self.user, price=100 + i * 50, currency='USD', year=2024, wish_details=f'Wish {i}', link='', place='General')
...
class UpdateWishlistStatusApiTest(TestCase):
    ...
    def setUp(self):
        ...
        self.wish = create_wish(user=self.user, price=200, currency='USD', year=2024, wish_details='Test', link='', place='General')
