...
class WishlistTransactionIntegrationTest(TransactionTestCase):
    ...
    def test_complete_wishlist_lifecycle(self):
        ...
        wish = create_wish(user=self.user, price=300, currency='USD', year=2024, wish_details='Test item', link='', place='General')
    ...
    def test_multiple_wishes_purchase(self):
        ...
        wish1 = create_wish(user=self.user, price=200, currency='USD', year=2024, wish_details='Item 1', link='', place='General')
        wish2 = create_wish(user=self.user, price=150, currency='USD', year=2024, wish_details='Item 2', link='', place='General')
        update_wish_status(user=self.user, wish_id=wish1.id)
        update_wish_status(user=self.user, wish_id=wish2.id)
        networth = NetWorth.objects.get(user=self.user, currency='USD')
        self.assertEqual(float(networth.total), 650)
