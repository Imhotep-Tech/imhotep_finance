...
class MonthlyReportHistoryApiTest(TestCase):
    ...
    def setUp(self):
        ...
        self.report = Reports.objects.create(user=self.user, month=1, year=2024, data=json.dumps({'user_deposit_on_range': [], 'user_withdraw_on_range': [], 'total_deposit': 0, 'total_withdraw': 0}))
...
class YearlyReportApiTest(TestCase):
    ...
    def setUp(self):
        ...
        for month in range(1, 4):
            Reports.objects.create(user=self.user, month=month, year=2024, data=json.dumps({'user_deposit_on_range': [{'category': 'Salary', 'converted_amount': 1000, 'percentage': 100}], 'user_withdraw_on_range': [], 'total_deposit': 1000, 'total_withdraw': 0}))
